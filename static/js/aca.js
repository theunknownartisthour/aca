...
$(document).ready(function () {
    $(window).scroll(function() {
        ...
        if (visibleHeight + currentScroll + 10 >= totalHeight) {
            ...
            if (window.location.pathname == '/the-archive' && $('#bookmark-end').length == 0) {
              console.log('get more articles');
              loadAjaxContent('#content', 'http://localhost:8087/the-archive?bookmark=2012-10-24%2004:29:24.280980', '#the-archive-next');
...
var loadAjaxContent = function(target, urlBase, selector) {
    $('#ajax_content').load(urlBase + ' ' + selector, function(response, status, xhr) {
        ...
        if (selector.match("-next$") == '-next') {
            $('div[id^="' + selector.slice(1, -5) + '"]').removeClass('hidden');
        }
        ajaxContentReady();
